...
int update_for_uboot(char *uboot_name)
{
	...
	if(!script_parser_fetch("hardware_version", "hid_used", value))
	{
		uboot_head->boot_data.multi_config_exist = value[0];
	}
	//修正存储设备信息
...
int reconstruct_uboot(char *buf, int length, char *script_buf, int script_length)
{
    ...
    script_head->length = script_align;
	printf("uboot length=%x, length=0x%x\n", length, total_length);
