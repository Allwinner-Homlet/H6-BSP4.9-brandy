...
int get_file_name(char *path, char *name)
{
	...
	memset(buffer_tmp, 0, MAX_PATH);
	if(!IsFullName(path))
	{
	   if(getcwd(buffer_tmp, MAX_PATH ) == NULL)
	   {
			perror( "getcwd error" );
			return -1;
	   }
	   sprintf(buffer, "%s/%s", buffer_tmp, path);
	}
	else
	{
		strcpy(buffer, path);
...
__s32 update_for_part_info(sunxi_mbr_t *mbr_info, sunxi_download_info *dl_map, __s32 mbr_count, __s32 mbr_size)
{
	...
	strncpy((char *)mbr_info->magic, SUNXI_MBR_MAGIC, 8);
	printf("mbr magic %s\n", mbr_info->magic);
	...
	strncpy((char *)dl_map->magic, SUNXI_MBR_MAGIC, 8);
	//根据分区情况会有变化的信息
	udisk_exist = 0;
	while(1)
	{
...
__s32 check_dl_size(char *filename,int part_size)
{
	...
	dl_file_size = ftell(dl_file);
	fseek(dl_file, 0, SEEK_SET);
	fclose(dl_file);
	printf("filename = %s \n",filename );
	printf("dl_file_size = %f sector\n",(float)dl_file_size / 512);
	printf("part_size = %d sector\n",part_size );
	if (dl_file_size > (part_size * 512))
	{
		return -1;
	}
	return 0;
