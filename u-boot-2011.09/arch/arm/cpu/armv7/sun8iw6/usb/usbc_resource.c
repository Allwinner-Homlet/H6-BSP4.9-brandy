...
int usb_open_clock(void)
{
    ...
    //open PHY clock: 1673 not the same with 1667
    reg_value = USBC_Readl(SUNXI_USBOTG_BASE+0x410);
	reg_value |=  (0x01 << 5);
    USBC_Writel(reg_value,SUNXI_USBOTG_BASE+0x410);
...
int SUBS_Dev_CoreReset(void)
{
	...
	reg_val = readl(USBC0_BASE + USBC_REG_o_PCTL);
	reg_val &= ~(0x01<<8);
	writel(reg_val, USBC0_BASE + USBC_REG_o_PCTL);
