...
{
	unsigned int to_cnt,retry_cnt,hdcp_hs_cnt;
	...
	hdmi_write(0x4845, video->is_yuv ? 0x80 : 0x00);
	hdmi_write(0xC845, video->is_yuv ? 0x80 : 0x00);
	hdmi_write(0xC844, 0x00);						
	hdmi_write(0x0840, 0x01);
...
hdcp_retry:
		...
		while(1)
		{
			if( (bsp_hdmi_get_hpd() != 1) && (retry_cnt < 5))
			{
				printf("hdcp retry***********\n");
				hdmi_udelay(100000);
				hdmi_write(0x00c1, 0x04);
			...
			if( (bsp_hdmi_get_hcts_sta() & 0xfe) != 0x40 )
			{	
				hdmi_udelay(200000);
				to_cnt--;
				if(to_cnt == 0)
...
unsigned char bsp_hdmi_get_hcts_sta()
{
	unsigned char ret = 0;
	hdmi_read(0x0);
  hdmi_write(0x10010,0x45);
